...
const LevelPageCategory = () => {
  ...
  const goToLevelDetail = (id, name) => {
    navigate(`/grammer_class/${id}`, { state : {id:id, name:name}}); // 이동할 URL 설정
  };
  ...
  return (
    <div>
      ...
      {levels.map((level) => (
          ...
          />
          <button onClick={() => goToLevelDetail(level.id, level.name)}>문제 풀러 가기</button>
          </div>
      ))}
      </div>
      <Footer></Footer>
...
export default LevelPageCategory;
