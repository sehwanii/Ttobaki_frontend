...
const QuizAudioRecorderPage = () => {
  ...
  const { question, currentQuestionIndex, questions, evaluations } = location.state;
  const leveldata = question;
  const [accessToken, setAccessToken] = useRecoilState(accessTokenState);
  const navigate = useNavigate();

  useEffect(() => {
    if (leveldata) {
      setMessage(leveldata.word);
    }
    setAudioUrl(null); // 문제 변경 시 오디오 URL 초기화
    setAudioBlob(null); // 문제 변경 시 오디오 Blob 초기화
  }, [leveldata]);
  ...
  const sendAudioToServer = async (audioBlob) => {
    ...
    try {
      ...
      const newEvaluations = [...evaluations, data];
      // 다음 문제로 이동
      if (currentQuestionIndex < 4) {
        const nextQuestionId = questions[currentQuestionIndex + 1];
        fetchNextQuestion(nextQuestionId, currentQuestionIndex + 1, newEvaluations);
      } else {
        navigate('/quiz-evaluation', { state: { evaluations: newEvaluations } });
      }
    } catch (error) {
    ...
    <div>
      <Nav></Nav>
        <div className="center-flex">
          <div>
            <h2>{`문제 ${currentQuestionIndex + 1} / 5`}</h2>
...
export default QuizAudioRecorderPage;
